...
namespace wordring
{
	...
	挿入される文字列リストが事前にソートされている場合、挿入速度、検索速度共に高速化される。
	ソートにかかる時間と挿入にかかる時間を足しても、ソートせずに挿入する時間に満たない。
	したがって、文字列リストを事前にソートしておくことが望ましい。
	検索速度に顕著な影響を与えることを考慮して事前のソートを推奨する。
	挿入順による影響は <b>std::set</b> と同様の特性を示す。

	ごく少量の文字列しか格納していない場合を除き、大部分が辞書順に挿入された条件下で、検索速度はどのSTLコンテナよりも速い。
	一方、挿入速度はどのSTLコンテナよりも遅い。
	*/
	...
	public:
		...
		*/
		...
		{
			return base_type::at(static_cast<typename base_type::const_iterator>(pos), idx);
		}
