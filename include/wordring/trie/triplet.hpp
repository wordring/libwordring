...
namespace wordring
{
	...
	public:
		using container = std::deque<std::deque<std::uint8_t>>;
	...
	public:
		...
		private:
			typename container::const_iterator m_basis;
		};
		...
		*/
		...
		public:
			...
			symbol_iterator(container::const_iterator const& current, int32_t depth)
				: m_current(current)
				, m_depth(depth) {}
		...
		*/
		...
		public:
			...
			/*! 有効な値を指すイテレータを作る
			*/
			node_iterator(container::const_iterator basis, uint32_t first, uint32_t last, int32_t depth)
				: m_basis(basis)
			...
			/*! 終端を指すイテレータを作る
			*/
			node_iterator(container::const_iterator basis, uint32_t last, int32_t depth)
				: m_basis(basis)
		...
		private:
			container::const_iterator m_basis; // m_list.begin()
			container::const_iterator m_it1;   // 現在の開始位置。
			container::const_iterator m_it2;   // 現在の終端位置。
			container::const_iterator m_last;  // 終端。
