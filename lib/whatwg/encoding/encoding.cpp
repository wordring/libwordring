...
static std::unordered_map<std::u32string, encoding_name> const encoding_label_map {
	// The Encoding
...
encoding_name wordring::whatwg::encoding::get_encoding_name(std::u32string label)
{
	...
	return it == encoding_label_map.end() ? static_cast<encoding_name>(0) : it->second;
}
